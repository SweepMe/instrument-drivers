...
def make_int_array_pointer(len_: int, values: list = None):
    ...
    values = [] makes an empty array pointer"""
    if values:
        array = (c.c_int32 * int(len_))(*values)
    else:
        array = (c.c_int32 * int(len_))()
...
def initialize():
    ...
    # _dll = c.cdll.LoadLibrary(_dll_path)
    ...
    # _dll.tstsel.restype = c.c_int32
    # _dll.devint.restype = c.c_int32
    # _dll.getinstid.restype = c.c_int32
    # _dll.setmode.restype = c.c_int32
    # _dll.limiti.restype = c.c_int32
    # _dll.limitv.restype = c.c_int32
    # _dll.rangev.restype = c.c_int32
    # _dll.rangei.restype = c.c_int32
    # _dll.lorangev.restype = c.c_int32
    # _dll.lorangei.restype = c.c_int32
    # _dll.forcev.restype = c.c_int32
    # _dll.forcei.restype = c.c_int32
    # _dll.intgv.restype = c.c_int32
    # _dll.intgi.restype = c.c_int32
...
def getinstid(id_string: str):
    ...
    # option 2
    # chars = create_string_buffer(string = id_string)
...
def arb_array(instr_id: int, chan: int, time_per_point: float, voltages: list, name: str):
    ...
    err = _dll.arb_array(c.c_int32(instr_id), c.c_int32(chan), c.c_double(time_per_point), len_c,
...
def seg_arb_define(instr_id: int, chan: int, num_segments: int, start_values: list,
                   stop_values: list, time_values: list, trig_values: list,
    ...
    err = _dll.seg_arb_define(c.c_int32(instr_id), c.c_int32(chan), c.c_int32(num_segments),
...
def pulse_fetch(instr_id: int, chan: int, start_index: int,
    ...
    err = _dll.pulse_fetch(c.c_int32(instr_id), c.c_int32(chan), c.c_int32(start_index),
                           ...
                           c.byref(timestamp), c.byref(status))
    get_error(err)
...
def pulse_meas_wfm(instr_id: int, chan: int, acquire_type: bool, acquire_meas_V: bool,
                   acquire_meas_I: bool, aquire_time_stamp: bool, load_line_effect_comp: bool):
...
def pulse_conncomp(instr_id: int, chan: int, comp_type: int, comp: int):
    ...
    err = _dll.pulse_conncomp(c.c_int32(instr_id), c.c_int32(chan), c.c_int32(comp_type),
                              c.c_int32(comp))
    get_error(err)
...
def pulse_meas_sm(instr_id: int, chan: int, acquire_type: int, acquire_meas_V_amp: bool,
                  acquire_meas_V_base: bool, acquire_meas_I_amp: bool, aquire_meas_I_base: bool,
                  aquire_time_stamp: bool, load_line_effect_comp: bool):
    ...
    err = _dll.pulse_meas_sm(c.c_int32(instr_id), chan, acquire_type, acquire_meas_V_amp,
                             acquire_meas_V_base, acquire_meas_I_amp, aquire_meas_I_base,
                             aquire_time_stamp, load_line_effect_comp)
    get_error(err)
...
def pulse_meas_rt(instr_id: int, chan: int, v_meas_name: str, i_meas_name: str,
                  timestamp_meas_name: str, status_meas_name: str):
...
def pulse_remove(instr_id: int, chan: int, voltage_to_output: float, output_relay_state: bool):
    ...
    err = _dll.pulse_remove(c.c_int32(instr_id), c.c_int32(chan), c.c_double(voltage_to_output),
                            c.c_int32(output_relay_state))
    get_error(err)
...
def pulse_source_timing(instr_id: int, chan: int, period: float, delay: float, width: float,
    ...
    err = _dll.pulse_source_timing(c.c_int32(instr_id), c.c_int32(chan), period, delay, width,
                                   rise, fall)
    get_error(err)
...
def seg_arb_sequence(instr_id: int, chan: int, sequence_id: int, num_segments,
                     start_voltages: List[float], stop_voltages: List[float], times: List[float],
                     trigger_values: List[bool], SSRs: List[int], meas_types: List[float],
                     meas_starts: List[float], meas_stops: List[float]):
...
def seg_arb_waveform(instr_id: int, chan: int, num_sequences: int, sequence_ids: List[int],
                     seq_loop_counts: List[float]):
    ...
    err = _dll.seg_arb_waveform(c.c_int32(instr_id), c.c_int32(chan), c.c_int32(num_sequences),
...
def decode_pulse_status(status_bits: int) -> dict:
    ...
    for cat, bit_len in bit_lengths.items():
        ...
        # print(shift, mask, status_bits >> shift & mask)
        shift += bit_len
