...
class LockstepMoveRequest:
    ...
    def validate(self) -> None:
        ...
        if self.arg_int is None:
            raise ValueError(f'Property "ArgInt" of "LockstepMoveRequest" is None.')

        if not isinstance(self.arg_int, (int, float, decimal.Decimal)):
            raise ValueError(f'Property "ArgInt" of "LockstepMoveRequest" is not a number.')

        if int(self.arg_int) != self.arg_int:
            raise ValueError(f'Property "ArgInt" of "LockstepMoveRequest" is not integer value.')
        ...
        if self.cyclic_direction is not None:
            if not isinstance(self.cyclic_direction, CyclicDirection):
                raise ValueError(f'Property "CyclicDirection" of "LockstepMoveRequest" is not an instance of "CyclicDirection".')

        if self.extra_cycles is not None:
            if not isinstance(self.extra_cycles, (int, float, decimal.Decimal)):
                raise ValueError(f'Property "ExtraCycles" of "LockstepMoveRequest" is not a number.')

            if int(self.extra_cycles) != self.extra_cycles:
                raise ValueError(f'Property "ExtraCycles" of "LockstepMoveRequest" is not integer value.')
