...
@dataclass
class PvtBufferGetSequenceDataResponse:

    sequence_data: List[PvtSequenceItem] = field(default_factory=list)

    @staticmethod
    def zero_values() -> 'PvtBufferGetSequenceDataResponse':
        return PvtBufferGetSequenceDataResponse(
            sequence_data=[],
        )

    @staticmethod
    def from_binary(data_bytes: bytes) -> 'PvtBufferGetSequenceDataResponse':
        """" Deserialize a binary representation of this class. """
        data = zaber_bson.loads(data_bytes)  # type: Dict[str, Any]
        return PvtBufferGetSequenceDataResponse.from_dict(data)
    ...
    @staticmethod
    def from_dict(data: Dict[str, Any]) -> 'PvtBufferGetSequenceDataResponse':
        return PvtBufferGetSequenceDataResponse(
    ...
    def validate(self) -> None:
        ...
        if self.sequence_data is not None:
            if not isinstance(self.sequence_data, Iterable):
                raise ValueError('Property "SequenceData" of "PvtBufferGetSequenceDataResponse" is not iterable.')

            for i, sequence_data_item in enumerate(self.sequence_data):
                if sequence_data_item is None:
                    raise ValueError(f'Item {i} in property "SequenceData" of "PvtBufferGetSequenceDataResponse" is None.')
