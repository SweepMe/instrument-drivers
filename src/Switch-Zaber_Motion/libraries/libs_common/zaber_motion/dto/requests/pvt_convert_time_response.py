...
@dataclass
class PvtConvertTimeResponse:

    sequence_data: List[PvtSequenceItem] = field(default_factory=list)

    @staticmethod
    def zero_values() -> 'PvtConvertTimeResponse':
        return PvtConvertTimeResponse(
            sequence_data=[],
        )

    @staticmethod
    def from_binary(data_bytes: bytes) -> 'PvtConvertTimeResponse':
        """" Deserialize a binary representation of this class. """
        data = zaber_bson.loads(data_bytes)  # type: Dict[str, Any]
        return PvtConvertTimeResponse.from_dict(data)
    ...
    @staticmethod
    def from_dict(data: Dict[str, Any]) -> 'PvtConvertTimeResponse':
        return PvtConvertTimeResponse(
    ...
    def validate(self) -> None:
        ...
        if self.sequence_data is not None:
            if not isinstance(self.sequence_data, Iterable):
                raise ValueError('Property "SequenceData" of "PvtConvertTimeResponse" is not iterable.')

            for i, sequence_data_item in enumerate(self.sequence_data):
                if sequence_data_item is None:
                    raise ValueError(f'Item {i} in property "SequenceData" of "PvtConvertTimeResponse" is None.')
