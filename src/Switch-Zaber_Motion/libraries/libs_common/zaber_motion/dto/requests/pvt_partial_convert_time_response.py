...
@dataclass
class PvtPartialConvertTimeResponse:

    sequence_data: List[PvtPartialSequenceItem] = field(default_factory=list)

    @staticmethod
    def zero_values() -> 'PvtPartialConvertTimeResponse':
        return PvtPartialConvertTimeResponse(
            sequence_data=[],
        )

    @staticmethod
    def from_binary(data_bytes: bytes) -> 'PvtPartialConvertTimeResponse':
        """" Deserialize a binary representation of this class. """
        data = zaber_bson.loads(data_bytes)  # type: Dict[str, Any]
        return PvtPartialConvertTimeResponse.from_dict(data)
    ...
    @staticmethod
    def from_dict(data: Dict[str, Any]) -> 'PvtPartialConvertTimeResponse':
        return PvtPartialConvertTimeResponse(
    ...
    def validate(self) -> None:
        ...
        if self.sequence_data is not None:
            if not isinstance(self.sequence_data, Iterable):
                raise ValueError('Property "SequenceData" of "PvtPartialConvertTimeResponse" is not iterable.')

            for i, sequence_data_item in enumerate(self.sequence_data):
                if sequence_data_item is None:
                    raise ValueError(f'Item {i} in property "SequenceData" of "PvtPartialConvertTimeResponse" is None.')
