    ...
    def validate(self) -> None:
        ...
        if self.sequence_items is not None:
            if not isinstance(self.sequence_items, Iterable):
                raise ValueError('Property "SequenceItems" of "PvtGeneratePositionsRequest" is not iterable.')

            for i, sequence_items_item in enumerate(self.sequence_items):
                if sequence_items_item is None:
                    raise ValueError(f'Item {i} in property "SequenceItems" of "PvtGeneratePositionsRequest" is None.')
