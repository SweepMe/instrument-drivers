...
* Initial action - Auto zero is an function sets the current level of each gain stage to zero, making signal changes easier to observe.  This function is performed before a measurement.
* Initial action - Auto zero reset cancels the automatic zeroing, and the signal returns to it's original level. This function is performed before a measurement.